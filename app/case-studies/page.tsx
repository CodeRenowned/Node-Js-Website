...
import { getCaseStudies, getFeaturedImage, getExcerpt, formatDate } from '@/lib/wordpress'

export const metadata: Metadata = {
  title: 'Case Studies — Proven SEO Results',
  description: 'See how CodeRenowned helps B2B brands grow organic traffic and revenue. Real data, real results across SaaS, FinTech, and more.',
}
...
export default async function CaseStudiesPage({ searchParams }: PageProps) {
  ...
  const { posts, totalPages } = await getCaseStudies(currentPage, 9)
      ...
      <section className="bg-gradient-to-br from-navy to-[#1d1b4c] py-16">
        <div className="container mx-auto px-5 text-center">
          ...
          <p className="text-white/60 font-body mt-4 max-w-xl mx-auto">
            Explore how we&apos;ve helped B2B brands drive measurable organic growth, authority, and revenue.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-5">
          {posts.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-gray-500 font-body text-lg">Case studies coming soon. Check back!</p>
            </div>
          ) : (
            <>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {posts.map((post) => (
                  <article key={post.id} className="card flex flex-col">
                    <Link href={`/case-studies/${post.slug}`} className="block -mx-6 -mt-6 mb-4 aspect-video overflow-hidden rounded-t-xl bg-gray-100">
                      <Image
              ...
              {totalPages > 1 && (
                <div className="flex justify-center gap-2 mt-12">
                  {currentPage > 1 && (
                    <Link href={`/case-studies?page=${currentPage - 1}`} className="px-4 py-2 rounded-lg border border-gray-200 text-sm font-body text-gray-600 hover:border-primary hover:text-primary transition-colors">
                      ← Previous
                    </Link>
                  )}
